...
class Openpagos
{
     ...
     * @return array
     */
    public function aplicar_cargo_con_tarjeta($costumer_id, $fuente_id, $cantidad, $descripcion, $dispositivo_id)
    {
        try {

            $customer = $this->openpay->customers->get($costumer_id);
            $customer->charges->create($this->_obtener_arreglo_para_cargo('card', $fuente_id, $cantidad, $descripcion, $dispositivo_id));
    ...
    {
        $mensaje = 'Ha ocurrido un error al intentar realizar el cargo, por favor inténtelo más tarde. '.$codigo_error;
