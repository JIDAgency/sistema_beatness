...
async function manejar_evento(clase_id, accion, funcion) {
	...
	mensaje_en_pantalla.innerHTML =
		'<i class="fa fa-spinner spinner"></i> Procesando...';
	...
	try {
			...
			if (row) {
				row
					.data({
						...
						id: updatedData.id,
						totalpass_eventOccurrenceUuid:
							updatedData.totalpass_eventOccurrenceUuid,
		...
		mostrar_error(
			"No se pudo procesar la solicitud en TotalPass.",
			button,
			accion
		);
	}
